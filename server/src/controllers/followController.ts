...
export const removeFollower = asyncHandler(
  async (
    ...
    const followeeId = req.user._id.toString();
    const followerId = req.params._id;

    if (followeeId === followerId) {
      throw new HttpError(
        HTTP_STATUS.BAD_REQUEST,
        USERS_MESSAGES.CANNOT_REMOVE_YOURSELF
      );
    }

    await FollowService.removeFollower(followeeId, followerId);
    res
      .status(HTTP_STATUS.OK)
      .json({ message: USERS_MESSAGES.REMOVE_FOLLOWER_SUCCESS });
  }
);
